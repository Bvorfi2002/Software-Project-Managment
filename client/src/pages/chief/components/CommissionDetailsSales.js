...
import { getCommissionsSalesAgent } from "../scripts/commission-scripts";
import { useSnackbar } from "notistack";

function calculateTierCommission(count){
    if(count >= 7){
        return count * 160;
    } else if(count >= 5){
        return count * 150;
...
function CommissionDetailsSales({ agentId, month, year }) {
      ...
      >
        <ModalContent sx={style}>
          ...
          >
            ...
            <p style={{fontSize: "8pt"}}><b>Number of sales: </b>{salesCount}</p>
            <p style={{fontSize: "8pt"}}><b>Total amount of tier commission: </b>{calculateTierCommission(salesCount)}</p>
            <MDBox pt={3}>
              <DataTable
                table={{ columns, rows }}
                isSorted={false}
...
export default CommissionDetailsSales;
